...
struct MANGOS_DLL_DECL boss_sacrolashAI : public ScriptedAI
{
    ...
    void KilledUnit(Unit *victim)
    {
        if(rand()%4 == 0)
        {
            ...
            case 0:
				...
				m_creature->MonsterYell(YELL_SAC_KILL_1 ,LANG_UNIVERSAL,NULL);
                break;
            case 1:
                DoPlaySoundToSet(m_creature,SOUND_SAC_KILL_2);
                m_creature->MonsterYell(YELL_SAC_KILL_2 ,LANG_UNIVERSAL,NULL);
                break;
            }
        }
    }
    ...
    void JustDied(Unit* Killer)
    {
        ...
        if (sisterdeath)
        {
            m_creature->MonsterYell(SAY_SAC_DEAD ,LANG_UNIVERSAL,NULL);
        }
        else
        {
    ...
    void UpdateAI(const uint32 diff)
    {
        if(!sisterdeath)
        {
            if (pInstance)
            {
                ...
                if (Temp && Temp->isDead())
                {
                    m_creature->MonsterYell(YELL_SISTER_ALYTHESS_DEAD ,LANG_UNIVERSAL,NULL);
            ...
            if(shadownova_timer < diff)
            {
                if (!m_creature->IsNonMeleeSpellCasted(false))
                {
                    ...
                    if(!sisterdeath)
                    {
						...
						m_creature->MonsterYell(YELL_SHADOW_NOVA,LANG_UNIVERSAL,NULL);
                    }
        ...
        {
            m_creature->InterruptSpell(CURRENT_GENERIC_SPELL);
			m_creature->MonsterYell(YELL_ENRAGE ,LANG_UNIVERSAL,NULL);
        ...
        if( m_creature->isAttackReady() && !m_creature->IsNonMeleeSpellCasted(false))
        {
			...
			if( m_creature->getVictim() && m_creature->IsWithinDistInMap(m_creature->getVictim(), 5))
            {
...
struct MANGOS_DLL_DECL boss_alythessAI : public Scripted_NoMovementAI
{
    ...
    void KilledUnit(Unit *victim)
    {
        if(rand()%4 == 0)
        {
            ...
            case 0:
				...
				m_creature->MonsterYell(YELL_ALY_KILL_1 ,LANG_UNIVERSAL,NULL);
                break;
            case 1:
                DoPlaySoundToSet(m_creature,SOUND_ALY_KILL_2);
                m_creature->MonsterYell(YELL_ALY_KILL_2 ,LANG_UNIVERSAL,NULL);
                break;
            }
        }
    }
    ...
    void JustDied(Unit* Killer)
    {
        if (sisterdeath)
        {
            m_creature->MonsterYell(YELL_ALY_DEAD ,LANG_UNIVERSAL,NULL);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(!sisterdeath)
        {
            if (pInstance)
            {
                ...
                if (Temp && Temp->isDead())
                {
                    m_creature->MonsterYell(YELL_SISTER_SACROLASH_DEAD ,LANG_UNIVERSAL,NULL);
            ...
            if (conflagration_timer < diff)
            {
                if (!m_creature->IsNonMeleeSpellCasted(false))
                {
                    ...
                    if(!sisterdeath)
                    {
						...
						m_creature->MonsterYell(YELL_CANFLAGRATION,LANG_UNIVERSAL,NULL);
                    }

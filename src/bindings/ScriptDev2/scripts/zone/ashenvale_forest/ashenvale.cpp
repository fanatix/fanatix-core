...
static GameObject* SelectGameObjectInGrid(Unit* pUnit, uint32 entry, float range)
{
    ...
    MaNGOS::NearestGameObjectEntryInObjectRangeCheck gameobject_check(*pUnit, entry, range);
    MaNGOS::GameObjectLastSearcher<MaNGOS::NearestGameObjectEntryInObjectRangeCheck> searcher(pUnit, pGameObject, gameobject_check);
    TypeContainerVisitor<MaNGOS::GameObjectLastSearcher<MaNGOS::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer> visitor(searcher);

    CellLock<GridReadGuard> cell_lock(cell, pair);
    cell_lock->Visit(cell_lock, visitor, *(pUnit->GetMap()));
...
struct MANGOS_DLL_DECL npc_ruul_snowhoofAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i)
    {
        ...
        case 0:    {
                ...
                GameObject* Cage = SelectGameObjectInGrid(m_creature, GO_CAGE, 20);
                if(Cage)
                    Cage->SetGoState(0);
                break;}
        case 13:

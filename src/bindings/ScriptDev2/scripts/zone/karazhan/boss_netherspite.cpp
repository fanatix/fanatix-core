			...
			if(uCandidate != uLastCandidate) {
				if(uBeamer) {
                    ...
                    if(uBeamerHelp) {
                        ...
                        uBeamerHelp = NULL;
                        uBeamerHelp = uBeamer->SummonCreature(creature_portal,RandomedCoords[0][0],RandomedCoords[0][1],RandomedCoords[0][2],0,TEMPSUMMON_CORPSE_DESPAWN,0);
    ...
    void AttackStart(Unit *who) {
        ...
        if(!PLRonRedBeam) {
            if (who->isTargetableForAttack() && who->isAlive() && who != m_creature) {
                DoStartMovement(who); /* after rev. 698: DoStartMovement() */
			...
			if(uTmp && uTmp->IsHostileTo(m_creature))
				DoStartMovement(uTmp); /* after rev. 698: DoStartMovement() */
        }
    }
        ...
        if(m_creature->getVictim() && m_creature->isAlive()) {
            ...
            if(PortalPhase) {
                ...
                if(Voidzone_timer < diff && !BanishPhase) {
                   ...
                   // I have no idea where the right value is
